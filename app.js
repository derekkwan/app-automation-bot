...
const BOT2_CHAT_ID = process.env.TELEGRAM_CHAT_ID_2 || BOT1_CHAT_ID; // Có thể dùng chung Chat ID
...
// Quản lý file JSON dữ liệu ví cho Bot 2
...
// ==========================================
// HÀM GỬI TIN NHẮN TELEGRAM (BOT 1 & BOT 2)
// ==========================================
...
async function getUSDTBalance(address) {
    try {
        // Kiểm tra địa chỉ ví TRC20 (bắt đầu bằng chữ T)
...
// Webhook nhận tin nhắn từ Bot 2
app.post('/telegram-webhook-bot2', async (req, res) => {
    ...
    // 4. Lệnh /get <tên_hoặc_nhóm_hoặc_địa_chỉ> (Tra cứu và lấy số dư trực tiếp)
    else if (command === '/get') {
        if (parts.length < 2) {
        ...
        if (matches.length === 0) {
            ...
            let msg = `🔍 *KẾT QUẢ TRA CỨU Cho: "${parts[1]}"*\n\n`;
...
async function checkWalletBalances() {
    ...
    for (let i = 0; i < wallets.length; i++) {
        ...
        if (newBalance !== null && newBalance !== w.balance) {
            ...
            // Cập nhật lại số dư mới vào bộ nhớ
...
// Chạy vòng lặp quét số dư ví 2 phút/lần
cron.schedule('*/2 * * * *', () => {
    checkWalletBalances();
});
...
// Server Endpoint
app.get('/', (req, res) => {
    res.send('Server Hệ Thống Đa Bot đang chạy!');
});
